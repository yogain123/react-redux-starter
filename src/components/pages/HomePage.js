...
import {getDetails} from '../state/actions/DetailsAction';
import DetailsHooks from '../../Hooks/DetailsHooks';


function HomePage(props){

    const resource = DetailsHooks("Changed State");
        ...
        return (
            ...
            <div className="jumbotron jumbotron-fluid text-dark bg-light animated fadeIn">
                ...
                <div className="text-center">
                    ...
                    <div>{props.DetailsInfo == "loading"? "loading" :props.DetailsInfo.name}</div>
                    <div>{resource}</div>
                    <button type="button" onClick={event=>props.getDetails()}>Click Me</button>
                </div>
            </div>
        </main>
        )
...
function mapStateToProps(states,ownProps){
    console.log(states);
    return {DetailsInfo : states.DetailsInfoReducers}
}

export default connect(mapStateToProps,{getDetails})(HomePage) ;